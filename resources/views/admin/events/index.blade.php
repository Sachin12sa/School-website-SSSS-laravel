        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 bg-slate-50/50">
                    <th class="text-left px-6 py-4 font-semibold text-navy-900 text-xs uppercase tracking-wide">Title</th>
                    <th
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($events as $event)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 font-medium text-navy-900">{{ Str::limit($event->title, 45) }}</td>
                        <td class="px-6 py-4 text-slate-500 hidden md:table-cell">{{ $event->start_date->format('d M Y') }}
                        </td>
                        <td class="px-6 py-4 text-slate-500 hidden lg:table-cell">{{ $event->location ?? '—' }}</td>
                        <td class="px-6 py-4">
                            <span
                                class="text-xs font-bold px-2.5 py-1 rounded-full {{ $event->is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1 justify-end">
                                ...
                                <form action="{{ route('admin.events.destroy', $event) }}" method="POST"
                                    onsubmit="return confirm('Delete this event?')">@csrf @method('DELETE')<button
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center text-slate-400">No events yet. <a
                                href="{{ route('admin.events.create') }}"
                                class="text-gold-500 hover:underline font-medium">Create the first one.</a></td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        @if ($events->hasPages())
            <div class="px-6 py-4 border-t border-slate-100">{{ $events->links() }}</div>
        @endif
    </div>
@endsection
